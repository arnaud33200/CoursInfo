...
void cinPoint( int & x, int & y )
{
	cout << "abs puis ord" << endl;
  	cin >> x >> y;
}

void cinIndice( int & ind )
{
	do
	{
		cout << "valeur de l'indice : " << endl;
		cin >> ind;
		if ( ind >= 100 || ind < 0 ) cout << "mauvaise entrée !" << endl << endl;
	}
	while ( ind >= 100 || ind < 0 );
}
...
void cinTriangle( char & col, Point & f, Point & s, Point & t)
{
	...
	cout << "premier Points ?  ";
	cinPoint( f );
	cout << "deuxieme Points ?  ";
	cinPoint( s );
	cout << "troisieme Points ?  ";
	cinPoint( t );
}
