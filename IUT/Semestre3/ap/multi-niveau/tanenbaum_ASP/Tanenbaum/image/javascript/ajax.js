...
function connectBan()
{
	...
	if (requeteBan != null)
	{
		requeteBan.open("GET", "requete/infogeneral.php", true);
...
//##########################################################################################
// AJAX POUR LES INFORMATIONS DU COMPTE
//##########################################################################################  

function connectCo()
{
	requeteCo = creerRequete();
	if (requeteCo != null)
	{
		requeteCo.open("GET","requete/affichercompte.php", true);
		requeteCo.onreadystatechange = handlerCo;
		requeteCo.send();
	}
	else
		window.alert("Pas de support AJAX (XMLHTTP)");
...
function handlerCo()
{
	if (requeteCo.readyState == 4 && requeteCo.status == 200)
		document.getElementById('infocompte').innerHTML = requeteCo.responseText;
} 
