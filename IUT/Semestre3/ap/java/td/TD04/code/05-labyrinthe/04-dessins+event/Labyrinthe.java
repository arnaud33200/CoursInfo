...
public class Labyrinthe {
    ...
    public Salle salleDown(Salle s){
	int[] coords = getCoordonneesSalle(s);
	if(coords[0]<hauteur-1) 
	    coords[0]++;
	if(salles[coords[0]][coords[1]]!=null)
	    return salles[coords[0]][coords[1]];
	else
	    return s;
    }	
    
    public Salle salleUp(Salle s){
	int[] coords = getCoordonneesSalle(s);
	if(coords[0]>0) 
	    coords[0]--;
	if(salles[coords[0]][coords[1]]!=null)
    ...
    public Salle salleLeft(Salle s){
	int[] coords = getCoordonneesSalle(s);
	if(coords[1]>0) 
	    coords[1]--;
	if(salles[coords[0]][coords[1]]!=null)
    ...
    public Salle salleRight(Salle s){
	int[] coords = getCoordonneesSalle(s);
	if(coords[1]<largeur-1) 
	    coords[1]++;
	if(salles[coords[0]][coords[1]]!=null)
	    return salles[coords[0]][coords[1]];
	else
