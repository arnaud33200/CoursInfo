...
########################################################################
#                                                                      #
#   Regroupe les fonctions qui permettent l'affichage et l'animation   #
#  du jeu.                                                             #
#                                                                      #
########################################################################
*/
...
/// texte 		
	...
	pastesurface (295,500,tab.temps,screen,NULL);
...
/// grille de diamant	
     for (int i=0; i<tab.G_SIZE; i++)
	{
	  for (int j=0; j<tab.G_SIZE; j++)
	    {
			if ( tab.matrice[i][j].sele==1 ) // Si une case est selectionnée.
				tab.matrice[i][j].lecture.y=61;
			else if (tab.matrice[i][j].cross==1) // Si une case est interdite.
				tab.matrice[i][j].lecture.y=122;
			else if (tab.matrice[i][j].blue==1) // Si une case est survolée par le curseur.
...
void gamepause ( board &tab, SDL_Surface *screen )
{
	...
	// Evite que le temps continue de s'écouler.
...
/************************************************************
 * Nom fonction : destruction colonne
 * Nom / Date : aladouce, 10/12/2010, v1
 * Description : Génère l'animation de destruction des diamants en colonne
 * Entrées : board tab, SDL_Surface screen
 * Sorties : board tab
************************************************************/
void destructioncolone (board &tab, SDL_Surface *screen)
{
	...
	for (int i=9; i>0; i--)   // Animation de l'explosion.
	{
		tab.le.x=i*60;
		j=tab.fi;
		while ( j<=tab.li && j<tab.G_SIZE )  // Parcoure les diamants alignés sans depasser la grille. 
		{
			SDL_Delay(0);
			pastesurface(tab.fj*60+tab.midleadd,j*60+tab.midleadd,tab.explosion,screen, &tab.le);
			j++;
		}	
		SDL_Delay(70); // Delai de temps entre chaque animation.
		SDL_Flip(screen);
	}
}
 ...
 * Nom / Date : aladouce, 10/12/2010, v1
 * Description : Génère l'animation de destruction des diamants en ligne
 * Entrées : board tab, SDL_Surface screen
 * Sorties : board tab
************************************************************/
void destructionligne (board &tab, SDL_Surface *screen)
{
	...
	for (int i=9; i>0; i--) // Animation de l'explosion.
	{
		tab.le.x=i*60;
		j=tab.fj;
		while (j<=tab.lj && j < tab.G_SIZE)// Parcoure les diamants alignés sans depasser la grille. 
		{
			SDL_Delay(0);
 ...
 * Nom / Date : aladouce, 10/12/2010, v1
 * Description : verifie si l'échange génère un alignement de 3 diamants ou plus
 * 				 et fait appel à diverses animations :
 * 				 	- explosions
 * 					- decalages
 * Entrées : board tab, SDL_Surface screen
 * Sorties : board tab
 ...
 * Description : verifie la présence de combos aprés un alignement
 * 				 et fait appel à diverses animations :
 * 				 	- explosions
 * 					- decalages
 * Entrées : board tab, SDL_Surface screen
