 ...
 * Description : affiche la fenêtre de fin de jeu aprés un abandon
...
void abandongame (SDL_Surface *screen, SDL_Event event)
{
	...
	SDL_WM_SetCaption("DIAMANT  (Partie Abandonnée !)", NULL);  
 ...
 * Nom fonction : finish game
 * Nom / Date : aladouce, 19/12/2010, v1
 * Description : Affiche la fenêtre de fin de jeu
 * Entrées : SDL_Surface screen et SDL_Event event
 * Sorties : rien
************************************************************/
void finishgame (SDL_Surface *screen, SDL_Event event)
{
	...
	SDL_WM_SetCaption("DIAMANT  (Partie Finie !)", NULL);  
	SDL_WM_SetIcon(icon, NULL);
