...
#   vérifications de la grille du jeu et des actions effectuées par     #
#   le joueur.                                                          #
#                                                                       #
#########################################################################
*/
 ...
 * Nom / Date : YGORRA Virgile  	/ 23/11/2010
 * Description : La fonction vérifie si trois diamants (ou plus) sont alignés horyzontalement :
 * 				 si oui: modification des coordonnées du premier et du dernier diamant aligné et retourne 1.
 * 				 sinon, retourne 0.
 * Entrées : board tab
 * Sorties : board tab
 ************************************************************/
bool solutionligne(board &tab)
{
  ...
  // Vérifie l'ordre des coordonnées reçues des cases echangées
  // de maniére que que la lecture de la grille se fasse dans le bon ordre.
	...
	while(i<=tab.li && !generale && i<tab.G_SIZE) // Parcoure seulement les lignes concernées.
    {
		cpt=1;
		for(int j=0; j<tab.G_SIZE-1; j++) // Parcoure toutes les colonnes.
		{
			if(tab.matrice[i][j].color == tab.matrice[i][j+1].color)
			{
				cpt++;
				if(cpt==3)  // Si trois diamants sont alignés.
				{
					tab.fj=j-1;
					tab.fi=i;
					tab.li=i;
				...
				else if (cpt>3)  // Si plus de trois diamants sont alignés.
				{
				tab.lj=tab.lj+1;
				tmp++;
				}
 ...
 * Description : La fonction vérifie si trois diamants (ou plus) sont alignés horizontalement
 * 				 si oui: modification des coordonnées du premier et du dernier diamant aligné et retourne 1.
 * 				 sinon, retourne 0.
 * Entrées : board tab
 * Sorties : board tab
...
bool solutioncolone(board &tab)
{
  ...
  // de maniére que que la lecture de la grille se fasse dans le bon ordre.
	...
	while(j<=tab.lj && !generale && j<tab.G_SIZE) // Parcoure seulement les deux colonnes concernées.
    {
		cpt=1;
 ...
 * Description : verifie si deux diamants sont côte à côte
 ...
 * Description : Vérifie s'il existe encore des solutions pour aligner trois
 * 				 diamants verticalement ou horizontalement. Cette fonction retourne 0 s'il
 * 				 n'y a pas de solution, et 1 s'il existe une ou plusieurs solutions. De plus,
...
bool verifgenerale(board & tab) 
{
	...
	while (!solution && l < tab.G_SIZE) 
    {
		...
		{
			// Teste si deux diamants sont alignés horyzontalement.
  ...
  c = 0; // On réinitialise c à 0, étant donné qu'il a gardé la valeur de la solution précédente.
	...
	while (!solution && c < tab.G_SIZE) 
    {
		...
		for (l = 0; l < tab.G_SIZE; l++) 
        {
            ...
            { 
				// Teste si deux diamants sont alignés verticalement.
 ...
 * Description : vérifie que la souris ne soit pas hors de la grille 
