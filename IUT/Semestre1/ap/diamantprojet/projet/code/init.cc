 ...
 * 				 (update1: precision des case a initialiser)
 * 				 (update2: verifi si la grille est bien initialisé)
...
{	
	...
	for ( int i=0; i < tab.G_SIZE; i++ ) 
	{
		for ( int j=0; j < tab.G_SIZE; j++ )
		{
			...
			// verification si il y a pas 3 diamants ou plus aligné	
			if ( j>1 )
 ...
 * Description : initialise tout les images utilisé dans le jeu
 ...
 * Nom / Date : aladouce, 12/12/2010, v1
 * Description : initialise les parametre du jeu en fonction du choix du joueur:
 * 				 - taille grille, nombre diamants, temps du jeu
 * 				 -> on definit les paramétre des trois niveaux différent
 * Entrées : structure board
 * Sorties : structure board
************************************************************/
