	 ...
	 unsigned long SampleCount) 
{
  ...
  // {
  //   if( *(pfInput1) < -1 ) *(pfOutput1++) = -1;
  //   else if( *(pfInput1) > 1 ) *(pfOutput1++) = 1;
  //   else if( *(pfInput1) >= -limit && *(pfInput1) <= limit )
  //     *(pfOutput1++) = 0;
  //   else
  //     *(pfOutput1++) = *(pfInput1);

  //   if( *(pfInput2) < -1 ) *(pfOutput2++) = -1;
  //   else if( *(pfInput2) > 1 ) *(pfOutput2++) = 1;
  //   else if( *(pfInput2) >= -limit && *(pfInput2) <= limit )
  //     *(pfOutput2++) = 0;
  //   else
  //     *(pfOutput2++) = *(pfInput2);
  ...
  for (i = 0; i < SampleCount; ++i)
  {
    // if(i == SampleCount-n0-1 )
    // {
    //   last_seq[0][SampleCount-i+n0-1] = *(pfInput1);
    //   last_seq[1][SampleCount-i+n0-1] = *(pfInput2);
    //   yes = 1;
    // }
    ...
    // else if( i-n0 < 0)
    // {
    //   *(pfOutput1++) = alpha * last_seq[0][i] + *(pfInput1++);
    //   *(pfOutput2++) = alpha * last_seq[1][i] + *(pfInput2++);
    // }
    if( i-n0 >= 0)
    {
      *(pfOutput1++) = alpha * *(pfInput1-n0) + *(pfInput1++);
      *(pfOutput2++) = alpha * *(pfInput2-n0) + *(pfInput2++);
    }
    else
    {
      *(pfOutput1++) = *(pfInput1++);
...
_init() 
{
  ...
  if (g_psDescriptor) 
  {
    ...
    g_psDescriptor->Maker
      = strdup("Licence Info Option Son");
