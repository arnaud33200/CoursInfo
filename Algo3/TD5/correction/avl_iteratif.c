...
tree tree_insert(tree node, keyfunc f, int key, void * object){
  ...
  while(current != NULL){
    stack_push(P,(void*) current);
    if(key < f(current->object))
...
tree tree_delete(tree node, int key, keyfunc f){
  ...
  if(current->left == NULL){
    ...
    current->object=tree_get_max(current->left);
    current->left=tree_delete_max(current->left);
    res=reequilibrage(current);
  }
